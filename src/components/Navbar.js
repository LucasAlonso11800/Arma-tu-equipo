import React, { useContext, useEffect } from 'react';
import { GlobalContext } from '../context/GlobalState';

function Navbar() {
    const [jugadores, setJugadores] = useContext(GlobalContext);
    
    const jugadoresSeleccion = [
        {
            nombre: 'Armani',
            imagen: 'https://images.ole.com.ar/collections/avatares/149575.jpg',
            id: 1
        },
        {
            nombre: 'Marchesin',
            imagen: 'https://images.ole.com.ar/collections/avatares/59891.jpg',
            id: 2
        },
        {
            nombre: 'Musso',
            imagen: 'https://images.ole.com.ar/collections/avatares/121537.jpg',
            id: 3
        },
        {
            nombre: 'Otamendi',
            imagen: 'https://images.ole.com.ar/collections/avatares/57410.jpg',
            id: 4
        },
        {
            nombre: 'Funes Mori',
            imagen: 'https://images.ole.com.ar/collections/avatares/121221.jpg',
            id: 5
        },
        {
            nombre: 'Pezzella',
            imagen: 'https://images.ole.com.ar/collections/avatares/121220.jpg',
            id: 6
        },
        {
            nombre: 'Casco',
            imagen: 'https://images.ole.com.ar/collections/avatares/62970.jpg',
            id: 7
        },
        {
            nombre: 'Tagliafico',
            imagen: 'https://images.ole.com.ar/collections/avatares/86479.jpg',
            id: 8
        },
        {
            nombre: 'Foyth',
            imagen: 'https://images.ole.com.ar/collections/avatares/234908.jpg',
            id: 9
        },
        {
            nombre: 'Saravia',
            imagen: 'https://images.ole.com.ar/collections/avatares/153394.jpg',
            id: 10
        },
        {
            nombre: 'Paredes',
            imagen: 'https://images.ole.com.ar/collections/avatares/91080.jpg',
            id: 11
        },
        {
            nombre: 'Rodriguez',
            imagen: 'https://images.ole.com.ar/collections/avatares/197024.jpg',
            id: 12
        },
        {
            nombre: 'Pizarro',
            imagen: 'https://images.ole.com.ar/collections/avatares/86261.jpg',
            id: 13
        },
        {
            nombre: 'Pereyra',
            imagen: 'https://images.ole.com.ar/collections/avatares/61566.jpg',
            id: 14
        },
        {
            nombre: 'De Paul',
            imagen: 'https://images.ole.com.ar/collections/avatares/119141.jpg',
            id: 15
        },
        {
            nombre: 'Lo Celso',
            imagen: 'https://images.ole.com.ar/collections/avatares/200826.jpg',
            id: 16
        },
        {
            nombre: 'Di Maria',
            imagen: 'https://images.ole.com.ar/collections/avatares/46497.jpg',
            id: 17
        },
        {
            nombre: 'Messi',
            imagen: 'https://images.ole.com.ar/collections/avatares/19054.jpg',
            id: 18
        },
        {
            nombre: 'Agüero',
            imagen: 'https://images.ole.com.ar/collections/avatares/37572.jpg',
            id: 19
        },
        {
            nombre: 'Dybala',
            imagen: 'https://images.ole.com.ar/collections/avatares/120638.jpg',
            id: 20
        },
        {
            nombre: 'Suarez',
            imagen: 'https://images.ole.com.ar/collections/avatares/51781.jpg',
            id: 21
        },
    ];
    const jugadoresRiver = [
        {
            nombre: 'Armani',
            imagen: 'https://images.ole.com.ar/collections/avatares/149575.jpg',
            id: 1
        },
        {
            nombre: 'Marchesin',
            imagen: 'https://images.ole.com.ar/collections/avatares/59891.jpg',
            id: 2
        },
        {
            nombre: 'Musso',
            imagen: 'https://images.ole.com.ar/collections/avatares/121537.jpg',
            id: 3
        },
    ]
    const jugadoresBoca = [
        {
            nombre: 'Armani',
            imagen: 'https://images.ole.com.ar/collections/avatares/149575.jpg',
            id: 1
        },
        {
            nombre: 'Marchesin',
            imagen: 'https://images.ole.com.ar/collections/avatares/59891.jpg',
            id: 2
        }
    ]

    useEffect(() => {
        setJugadores(jugadoresSeleccion)
    }, []);

    return (
        <div className="navbar">
            <h2>Arma tu equipo</h2>
            <ul className="lista">
                <li className="item-lista" onClick={() => setJugadores(jugadoresSeleccion)}>Seleccion Argentina</li>
                <li className="item-lista" onClick={() => setJugadores(jugadoresRiver)}>River</li>
                <li className="item-lista" onClick={() => setJugadores(jugadoresBoca)}>Boca</li>
            </ul>
        </div>
    )
}

export default Navbar
